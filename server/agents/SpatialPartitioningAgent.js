...
class SpatialPartitioningAgent {
  constructor(worldWidth, worldHeight, gridSize = 200) {
    ...
    console.log(
      `🗂️ Spatial Partitioning Agent initialized: ${this.cols}x${this.rows} grid (${gridSize}px cells)`
    );
  }
  ...
  getCellKey(x, y) {
    const col = Math.floor(
      Math.max(0, Math.min(x, this.worldWidth - 1)) / this.gridSize
    );
    const row = Math.floor(
      Math.max(0, Math.min(y, this.worldHeight - 1)) / this.gridSize
    );
    return `${col},${row}`;
  }
  ...
  getCellsInBounds(x, y, width, height) {
    ...
    const endCol = Math.floor(
      Math.min(this.worldWidth - 1, x + width) / this.gridSize
    );
    const startRow = Math.floor(Math.max(0, y) / this.gridSize);
    const endRow = Math.floor(
      Math.min(this.worldHeight - 1, y + height) / this.gridSize
    );
  ...
  getObjectsInViewport(
    ...
    const cells = this.getCellsInBounds(
      viewportX,
      viewportY,
      viewportWidth,
      viewportHeight
  ...
  getObjectsNear(x, y, radius, objectTypes = null) {
    ...
    return this.getObjectsInViewport(
      viewportX,
      viewportY,
      viewportWidth,
      viewportHeight,
