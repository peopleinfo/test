...
class BinaryProtocol {
  ...
  createOptimizedGameState(playerId, currentState, batchedUpdates = null) {
    ...
    const fullSize = JSON.stringify(
      this.optimizeStateData(currentState)
    ).length;
  ...
  optimizeStateData(state) {
    const optimized = {
      players:
        state.players?.map((player) => this.optimizePlayer(player)) || [],
      foods: state.foods?.map((food) => this.optimizeFood(food)) || [],
      deadPoints:
        state.deadPoints?.map((point) => this.optimizeDeadPoint(point)) || [],
    };
  ...
  optimizePlayer(player) {
    return {
      ...
      p: player.points
        ? player.points.slice(0, 50).map((point) => ({
  ...
  createDelta(previous, current) {
    ...
    if (current.players) {
      ...
      current.players.forEach((player) => {
        ...
        if (!prevPlayer) {
          ...
          playerDeltas.push({
            action: "add",
            data: this.optimizePlayer(player),
          });
        } else {
          // Check for changes
    ...
    if (current.deadPoints && previous.deadPoints) {
      const deadPointChanges = this.getDeadPointChanges(
        previous.deadPoints,
        current.deadPoints
      );
  ...
  createEnhancedDelta(previous, current) {
    ...
    if (current.players) {
      ...
      current.players.forEach((player) => {
        ...
        if (!prevPlayer) {
          ...
          const positionDelta = this.getEnhancedPlayerChanges(
            prevPlayer,
            player
          );
    ...
    if (current.foods && previous.foods) {
      const foodChanges = this.getEnhancedFoodChanges(
        previous.foods,
        current.foods
      );
      if (foodChanges.length > 0) {
    ...
    if (current.deadPoints && previous.deadPoints) {
      const deadPointChanges = this.getEnhancedDeadPointChanges(
        previous.deadPoints,
        current.deadPoints
      );
      if (deadPointChanges.length > 0) {
  ...
  getEnhancedPlayerChanges(prev, curr) {
    ...
    if (Math.abs(radiusDiff) > 0.5) {
      // Only if radius changed significantly
  ...
  createBatchedPlayerUpdate(playerUpdates) {
    ...
    for (const update of playerUpdates) {
      ...
      if (Object.keys(deltaUpdate).length > 1) {
        // More than just ID
  ...
  getDeadPointChanges(prevPoints, currPoints) {
    ...
    currPoints.forEach((currPoint) => {
      if (!prevPoints.find((p) => p.id === currPoint.id)) {
        changes.push({
          action: "add",
          data: this.optimizeDeadPoint(currPoint),
        });
      }
    });
    ...
    return {
      ...
      deltaRatio:
        this.stats.totalUpdates > 0
          ? (this.stats.deltaUpdates / this.stats.totalUpdates) * 100
          : 0,
